...
handles.index = sort(handles.index(:));
if size(handles.index,1)>1
...
handles.inddis      =   union(handles.indnomeeg,handles.indexMEEG);
if size(handles.inddis,1)>1
    handles.inddis = handles.inddis';
end
...
if isfield(D,'CRC')
    if isfield(D.CRC,'DC')
        if isfield(D.CRC.DC,'shortartf')
            handles.artefact = D.CRC.DC.shortartf.artefact;
            handles.arousal = D.CRC.DC.shortartf.arousal;
        end
    end
end
% badchannels
NofW    =   ceil(nsamples(D)/(fsample(D)*30));
handles.badchannels = cell(NofW,1);
if isfield(D,'CRC')
    if isfield(D.CRC,'DC')
        if isfield(D.CRC.DC,'badchannels')
            if isfield(D.CRC.DC.badchannels,'chan_defaillant')
                for ibc = 1 : NofW
                    handles.badchannels{ibc} = union(D.CRC.DC.badchannels.chan_defaillant{ibc},D.CRC.DC.badchannels.chan_incoherent{ibc});
                end;
            end
        end
    end
...
if ~handles.multcomp
    ...
    if handles.scoring
        ...
        crc_hypnoplot(handles.axes4,handles,handles.winsize)
        ...
        set(handles.nextart,'visible','off')
        set(handles.prevart,'visible','off')
        set(handles.stopart,'visible','off')
    end
end
D   =   handles.Dmeg{1};
...
if ~isempty(aevt)
    ...
    if isfield(D.CRC,'spindles')
        ...
        if isfield(D.CRC.spindles,'bounds')
...
if norm
    ...
    handles.indexMEEG = setdiff(handles.indexMEEG, handles.nind);
else 
...
if oksamples && okchannels && okscore
    ...
    hPlotsPopupmenu = uicontrol(... % List of available types of plot
        ...
        'HandleVisibility','callback', ...
        'String',sc,...
        'Callback', {@hypnoplotimport,handles,Mainhandle},...
        'Style','popupmenu');
    ...
    crc_hypnoplot(hPlotAxes,handles,D.CRC.score{3,1},D.CRC.score{1,1})
...
function hypnoplotimport(hObject, eventdata, handles, gcbo)
...
if size(D.CRC.score,2)>=Valpop
    crc_hypnoplot(axesidx,handles,D.CRC.score{3,Valpop},D.CRC.score{1,Valpop},D.CRC.score{2,Valpop})
end
...
handles.popcmp(1) = uicontrol(... % List of available types of plot
    ...
    'Callback', {@hypnoplotcompare,handles,Mainhandle},...
    'Style','popupmenu');
...
crc_hypnoplot(handles.subcmp(3),...
    handles,...
...
function hypnoplotcompare(hObject, eventdata, handles, gcbo)
...
% --------------------------------------------------------------------
function autoart_Callback(hObject, eventdata, handles)
% hObject    handle to autoart (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

set(handles.prevart,'enable','on','visible','on')
set(handles.nextart,'enable','on','visible','on')
set(handles.stopart,'enable','on','visible','on')  
auto = handles.Dmeg{1}.CRC.DC.shortartf.total;
da = find(diff(auto)~=1);
fina = sort([auto(da) auto(end)]);
deba = sort([auto(1) auto(da+1)]);
if size(handles.Dmeg{1}.CRC.score{5,:},2) == 3
    qart = handles.Dmeg{1}.CRC.score{5,:};
    ind = find(qart(:,3)==0);
    artefacts = qart(ind,1:2);
else 
    artefacts =handles.Dmeg{1}.CRC.score{5,:};
    end
if ~isempty(artefacts)
    if ~isempty(handles.Dmeg{1}.CRC.score{6,:})
        expert = union(artefacts,handles.Dmeg{1}.CRC.score{6,:},'rows')';
    else 
        expert = artefacts';
end
else
    if ~isempty(handles.Dmeg{1}.CRC.score{6,:})
        expert = handles.Dmeg{1}.CRC.score{6,:}';
    else 
        expert = artefacts';
    end
end
art = [];
wa = [];
for ie = 1 : size(expert,2)
    art = [art ceil(expert(1,ie)) : ceil(expert(2,ie))];
end
for iii = 1 : length(deba)
    eva = deba(iii) : fina(iii);
    ca = intersect(eva, art);
    if isempty(ca)
        wa = union(wa,unique(ceil(deba(iii)/handles.winsize)));
    end
end
% wa = unique(ceil(deba/handles.winsize));
handles.art = wa;%ceil(deba/handles.winsize);
% s=[];
% for w = 1 : length(wa)
%     if length(find(wa(w)==handles.art))>2
%         s = union(s,wa(w));
%     end
% end
% handles.art = s;
if isfield(handles.Dmeg{1},'CRC')
    if isfield(handles.Dmeg{1}.CRC,'score')
        sc = find(and(handles.Dmeg{1}.CRC.score{1} > 1,handles.Dmeg{1}.CRC.score{1} <8));
        handles.art = intersect(handles.art,sc);
    end
end
handles.indart = 1;
guidata(hObject, handles);

function stopart_Callback(hObject, eventdata, handles)
% hObject    handle to stopart (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

set(handles.prevart,'enable','off','visible','off')
set(handles.nextart,'enable','off','visible','off')
set(handles.stopart,'enable','off','visible','off')
handles.indart = 1;
guidata(hObject, handles);

function expart_Callback(hObject, eventdata, handles)
% hObject    handle to autoart (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
set(handles.stopart,'enable','on','visible','on')
if size(handles.Dmeg{1}.CRC.score{5,:},2) == 3
    ind = find(handles.Dmeg{1}.CRC.score{5,:}(:,3)==0);
    artefacts = handles.Dmeg{1}.CRC.score{5,:}(ind,1:2);
else 
    artefacts =handles.Dmeg{1}.CRC.score{5,:};
end
if ~isempty(artefacts)
...
wa = [];
for ie = 1 : size(expert,2)
    art = ceil(expert(1,ie)) : ceil(expert(2,ie));
    ca = intersect(art,auto);
    if isempty(ca)
        wa = union(wa,ceil(art/handles.winsize));
end
end
handles.art = wa;%unique(ceil(art/handles.winsize));
handles.indart = 1;
guidata(hObject, handles);

function prevart_Callback(hObject, eventdata, handles) 
if handles.indart>1
    handles.indart = handles.indart - 1;
end
set(handles.slider1,'Value',(handles.art(handles.indart)-1)*handles.winsize);
guidata(hObject, handles);
mainplot(handles)


function nextart_Callback(hObject, eventdata, handles)
if handles.indart ~= length(handles.art)
    handles.indart = handles.indart + 1;
end
if ~isempty(handles.art)
set(handles.slider1,'Value',(handles.art(handles.indart)-1)*handles.winsize);
guidata(hObject, handles);
mainplot(handles)
end
...
for i=1:maxi
    ...
    if handles.multcomp
        ...
        slidpos     = Chanslidval-rem(Chanslidval,1);
        NbreChandisp    = str2double(get(handles.NbreChan,'String'));
        index   = union(handles.indnomeeg,handles.indexMEEG);
        handles.inddis = index(slidpos : 1 : min(slidpos + NbreChandisp -1,length(index)));
        ...
        win =  ceil(str2double(get(handles.currenttime,'String'))/handles.winsize);
   end
...
%---- depuis 20/6/2013
          if  ~isempty(handles.artefact)
                ...
                for epo     =   1 : length(deb_epoch)  
                    X   =   get(plt,'YData'); 
                    deb =  max((handles.artefact(deb_epoch(epo)) - (win-1)*handles.winsize - 1)*fsample(handles.Dmeg{1}),1); %1=temps d'une époque artefactée
                    fin =  min(length(X),(handles.artefact(deb_epoch(epo)) - (win-1)*handles.winsize)*fsample(handles.Dmeg{1}));
                    time_epo = [deb/fsample(handles.Dmeg{1}) : 1/fsample(handles.Dmeg{1}) : fin/fsample(handles.Dmeg{1})] + (win-1)*handles.winsize;
                    X   =   X(deb:fin);%fs est le nombre d'échantillons contenus dans une seconde
                    plot(time_epo,X,'-','color',[0.8 0 0]);
                end
            end   
            if  ~isempty(handles.arousal)
                deb_epoch   =   find(and(handles.arousal>temps(1),handles.arousal<=temps(end)+1/fsample(handles.Dmeg{1})));%1 seconde = fenetre d'analyse pour la détection des artefacts de courtes durées
                for epo     =   1 : length(deb_epoch)  
                    X   =   get(plt,'YData'); 
                    deb =  max((handles.arousal(deb_epoch(epo)) - (win-1)*handles.winsize - 1)*fsample(handles.Dmeg{1}),1); %1=temps d'une époque artefactée
                    fin =  min(length(X),(handles.arousal(deb_epoch(epo)) - (win-1)*handles.winsize)*fsample(handles.Dmeg{1}));
                    time_epo = [deb/fsample(handles.Dmeg{1}) : 1/fsample(handles.Dmeg{1}) : fin/fsample(handles.Dmeg{1})] + (win-1)*handles.winsize;
                    X   =   X(deb:fin);%fs est le nombre d'échantillons contenus dans une seconde
                    plot(time_epo,X,'-','color',[0 0.8 0]);
                end
            end
            ...
            if handles.scoring && ~isempty(handles.score{5,handles.currentscore}) && any(index(j)== handles.score{5,handles.currentscore}(:,3))
                ...
                while l<=length(indice) 
                    if(or(or(handles.score{5,handles.currentscore}(indice(l),2)>tdebs,handles.score{5,handles.currentscore}(indice(l),1)<tdebs),...
                        ...
                        if handles.export
                            plot(time,X,'Color',[1 0 0]);
                        else  
                            plot(time,X,'UIContextMenu',handles.Deletemenuone,'Color',[1 0 0]);
                        end 
            ...
            if ~isempty(handles.badchannels)
                win15 = ceil((win-1)*handles.winsize/30+1):ceil(win*handles.winsize/30);
                for iw15 = 1 : numel(win15)
                    el = handles.badchannels{win15(iw15)};  
                    if any(el==index(j))
                        for itm = 1:length(el)
                            X   =   get(plt,'YData');
                            deb =  1; 
                            fin =  min(length(X),handles.winsize*fsample(handles.Dmeg{1}));
                            time_epo = [(win-1)*handles.winsize + 1/fsample(handles.Dmeg{1}) : 1/fsample(handles.Dmeg{1}) : win*handles.winsize ];
                            X = X(deb:fin);
                            plot(time_epo,X,'-r');
                        end
                    end
...
if handles.scoring && handles.winsize == handles.score{3,handles.currentscore} % New gardian to check the window size corresponds to those is used to score file
    ...
    if length(temps)-round(0.8*fsample(handles.Dmeg{1}))>0
        text(temps(length(temps)-round(0.8*fsample(handles.Dmeg{1}))),handles.scale*(fact+5/8),num2str(curscore),'Color','k','FontSize',14)
    end
    switch  curscore
        case    0
...
%%%%%%%%%%%%%%%Changement de base 2 vers 10 pour les triggers  %%%%%%%%%%
