...
%   May 26, 2009
%   replace prcorr2 with sub-function
%   C. Phillips, CRC, ULg, Belgium
...
for s=1:sections
    ...
    sec_P=find(sec==1);
    sec_dP=diff(sec_P); 
    if isempty(sec_dP); %added by pm 30/06/2005 (cases where sec_P is 1x1)
        sec_dP=0;
    end
    dP_med=median(sec_dP);
...
% R = tmp(2);
% return;
% % Original function from R. Niazy in fMRIB.
% % It could/should be optimized as corrcoef is very inefficient for most
% % usual cases.

function R = prcorr2(A,B)
% Calculate correlation coefficient C between inputs A and B
% This implementation works on 2D arrays and is faster than corrcoef.m from
% matlab
mnA = A;
mnB = B;

while max(size(mnA))>1
    mnA = mean(A);
end

while max(size(mnB))>1
    mnB = mean(B);
end

% The previous modification avoid the use of mean2
% A = A - mean2(A);
% B = B - mean2(B);
