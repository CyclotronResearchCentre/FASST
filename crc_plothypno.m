function crc_plothypno(hand,pl,handles,windowsize,score,scorer)

% FORMAT crc_plothypno(hand,pl,handles,windowsize,score,scorer)
...
if ~isempty(art)
    art=art(:,1);
    plot(art,ones(length(art),1)*8,'+','MarkerSize',8,'MarkerFaceColor',[0.4 0.4 0.4],...
        'MarkerEdgeColor',[0.4 0.4 0.4],'tag','undart')
end
if ~isempty(arou)
    arou=arou(:,1);
    plot(arou,ones(length(arou),1)*8,'+','MarkerSize',8,'MarkerFaceColor',[1 0 0],...
        'MarkerEdgeColor',[1 0 0],'tag','arou')
end
if ~isempty(eoi)
    eoi=eoi(:,1);
    plot(eoi,ones(length(eoi),1)*8,'+','MarkerSize',8,'MarkerFaceColor',[0.75 0.2 0.2],...
...
if ~isempty(evtot)
    ...
    for evm = 1 : nevm
        ...
        indcol = col(1);
        if ~isempty(evdis)
            plot(evdis,ones(length(evdis),1)*8,'V','MarkerSize',5,'MarkerFaceColor',indcol,...
...
if isfield(handles,'slider1')
    ...
    handles.cursor=plot(pos,8,'v','Color',[0.2 0.2 0.2],'LineWidth',2.5,'tag','cursor');
    hold off
end
...
if isfield(handles,'scoring') && handles.scoring
    ...
    text(totime-(9*totime/100),7.5,scorer,'Color',[0.3 0.3 0.3],'Fontweight','demi')
end
